const pJS = function (e) {
    ...
    var t = {
        ...
        particles: {
            ...
            size: {value: 10, random: !0, anim: {enable: !1, speed: 80, size_min: .1, sync: !1}},
            line_linked: {enable: !0, distance: 300, color: "#ff9800", opacity: .4, width: 2},
            move: {
                enable: !0,
                speed: 12,
                direction: "none",
        ...
        interactivity: {
            ...
            modes: {
                ...
                bubble: {distance: 800, size: 80, duration: 2, opacity: .8, speed: 3},
    ...
    t.tmp.obj = {
        ...
        this.radius = (t.particles.size.random ? Math.random() : 1) * t.particles.size.value, this.x = i ? i.x : Math.random() * t.canvas.w, this.y = i ? i.y : Math.random() * t.canvas.h, this.x > t.canvas.w - 2 * this.radius ? this.x = this.x - this.radius : this.x < 2 * this.radius && (this.x = this.x + this.radius), this.y > t.canvas.h - 2 * this.radius ? this.y = this.y - this.radius : this.y < 2 * this.radius && (this.y = this.y + this.radius), this.color = {}, this.color = e, this.color.rgb = hexToRgb(this.color.value), this.opacity = (t.particles.opacity.random ? Math.random() : 1) * t.particles.opacity.value, this.vx = Math.random() - .5, this.vy = Math.random() - .5, this.vx_i = this.vx, this.vy_i = this.vy, this.shape = t.particles.shape.type
    }, t.fn.particle.prototype.draw = function () {
        var e = this, a = e.radius, i = e.opacity,
            n = "rgba(" + e.color.rgb.r + "," + e.color.rgb.g + "," + e.color.rgb.b + "," + i + ")";
        t.canvas.ctx.fillStyle = n, t.canvas.ctx.beginPath(), t.canvas.ctx.arc(e.x, e.y, a, 0, 2 * Math.PI, !1), t.canvas.ctx.closePath(), t.canvas.ctx.fill()
        ...
        for (var e = 0; e < t.particles.array.length; e++) {
            ...
            if (a.x - a.radius > t.canvas.w ? (a.x = r.x_left, a.y = Math.random() * t.canvas.h) : a.x + a.radius < 0 && (a.x = r.x_right, a.y = Math.random() * t.canvas.h), a.y - a.radius > t.canvas.h ? (a.y = r.y_top, a.x = Math.random() * t.canvas.w) : a.y + a.radius < 0 && (a.y = r.y_bottom, a.x = Math.random() * t.canvas.w), t.fn.modes.grabParticle(a), t.particles.line_linked.enable) for (var n = e + 1; n < t.particles.array.length; n++) {
        ...
        for (var e = 0; e < t.particles.array.length; e++) t.particles.array[e].draw()
    }, t.fn.interact.linkParticles = function (e, a) {
        var i = e.x - a.x, n = e.y - a.y, s = Math.sqrt(i * i + n * n);
        if (s <= t.particles.line_linked.distance) {
            var r = t.particles.line_linked.opacity - s / (1 / t.particles.line_linked.opacity) / t.particles.line_linked.distance;
            if (r > 0) {
                var c = t.particles.line_linked.color_rgb_line;
                t.canvas.ctx.strokeStyle = "rgba(" + c.r + "," + c.g + "," + c.b + "," + r + ")", t.canvas.ctx.lineWidth = t.particles.line_linked.width, t.canvas.ctx.beginPath(), t.canvas.ctx.moveTo(e.x, e.y), t.canvas.ctx.lineTo(a.x, a.y), t.canvas.ctx.stroke(), t.canvas.ctx.closePath()
            }
        }
    }, t.fn.modes.pushParticles = function (e, a) {
        t.tmp.pushing = !0;
        for (var i = 0; i < e; i++) t.particles.array.push(new t.fn.particle(t.particles.color, t.particles.opacity.value, {
            x: a ? a.pos_x : Math.random() * t.canvas.w,
            y: a ? a.pos_y : Math.random() * t.canvas.h
        }))
    }, t.fn.modes.removeParticles = function (e) {
        t.particles.array.splice(0, e), t.particles.move.enable || t.fn.particlesDraw()
    }, t.fn.modes.grabParticle = function (e) {
        if (t.interactivity.events.onhover.enable && "mousemove" === t.interactivity.status) {
            var a = e.x - t.interactivity.mouse.pos_x, i = e.y - t.interactivity.mouse.pos_y,
                n = Math.sqrt(a * a + i * i);
            if (n <= t.interactivity.modes.grab.distance) {
                ...
                if (s > 0) {
                    ...
                    t.canvas.ctx.strokeStyle = "rgba(" + r.r + "," + r.g + "," + r.b + "," + s + ")", t.canvas.ctx.lineWidth = t.particles.line_linked.width, t.canvas.ctx.beginPath(), t.canvas.ctx.moveTo(e.x, e.y), t.canvas.ctx.lineTo(t.interactivity.mouse.pos_x, t.interactivity.mouse.pos_y), t.canvas.ctx.stroke(), t.canvas.ctx.closePath()
                }
            }
        }
    }, t.fn.vendors.eventsListeners = function () {
        "window" === t.interactivity.detect_on ? t.interactivity.el = window : t.interactivity.el = t.canvas.el, (t.interactivity.events.onhover.enable || t.interactivity.events.onclick.enable) && (t.interactivity.el.addEventListener("mousemove", function (e) {
            let a = e.offsetX || e.clientX, i = e.offsetY || e.clientY;
            t.interactivity.mouse.pos_x = a, t.interactivity.mouse.pos_y = i, t.tmp.retina && (t.interactivity.mouse.pos_x *= t.canvas.pxratio, t.interactivity.mouse.pos_y *= t.canvas.pxratio), t.interactivity.status = "mousemove"
        ...
        if (t.particles.number.density.enable) {
            ...
            var a = e * t.particles.number.value / t.particles.number.density.value_area,
                i = t.particles.array.length - a;
            i < 0 ? t.fn.modes.pushParticles(Math.abs(i)) : t.fn.modes.removeParticles(i)
        }
...
}, window.cancelRequestAnimFrame = window.cancelAnimationFrame || window.webkitCancelRequestAnimationFrame || window.mozCancelRequestAnimationFrame || window.oCancelRequestAnimationFrame || window.msCancelRequestAnimationFrame || clearTimeout, window.pJSDom = [], window.particlesJS = function (e) {
    document.getElementById(e).getElementsByClassName("particles-js-canvas-el");
    var a = document.createElement("canvas");
    a.className = "particles-js-canvas-el", a.style.width = "100%", a.style.height = "100%", a.style.position = "absolute", a.style.top = "0px", null !== document.getElementById(e).appendChild(a) && pJSDom.push(new pJS(e))
